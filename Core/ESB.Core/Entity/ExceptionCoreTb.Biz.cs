...
namespace ESB.Core.Entity
{
    ...
    public class ExceptionCoreTb : ExceptionCoreTb<ExceptionCoreTb> { }
    
    /// <summary></summary>
    public partial class ExceptionCoreTb<TEntity> : Entity<TEntity> where TEntity : ExceptionCoreTb<TEntity>, new()
    {
        #region 对象操作﻿
        static ExceptionCoreTb()
        {
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("开始初始化{0}ExceptionCoreTb数据……", typeof(TEntity).Name);

        //    var entity = new ExceptionCoreTb();
        //    entity.Name = "admin";
        ...
        //    if (XTrace.Debug) XTrace.WriteLine("完成初始化{0}ExceptionCoreTb数据！", typeof(TEntity).Name);
        //}
        #endregion
        ...
        public static TEntity FindByExceptionID(Guid exceptionid)
        {
            if (Meta.Count >= 1000)
                ...
                return Meta.Cache.Entities.Find(_.ExceptionID, exceptionid);
            // 单对象缓存
            //return Meta.SingleCache[exceptionid];
        }
        #endregion

...
namespace CoreWeb.Core
{
    public class SoapClient
    {
        ...
        /// <returns></returns>
        internal static JN.ESB.Schemas.服务响应 CallWebService(CallState state)
        {
            CallState callState = state as CallState;
            BindingTemplate binding = callState.Binding;
            JN.ESB.Schemas.服务请求 request = callState.Request;
            ...
            if (soapClient != null)
            {
                ...
                JN.ESB.Schemas.服务响应 response = GetResponse(res, method.ReturnType);
                LogUtil.AddAuditLog(
                    1
                    , binding
        ...
        /// <returns></returns>
        private static JN.ESB.Schemas.服务响应 GetResponse(object target, Type type)
        {
